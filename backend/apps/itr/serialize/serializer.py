...
plain dict (see itr/model_blank.py).

Every element name below has been checked against
`itr/data/ay2026-27/ITR-1_2026_Main_V1_1.json`. Where this module's naming or
arithmetic differs from the interpretive build prompt, the schema wins.
...
from apps.itr.engine.constants import CONSTANTS as Constants
from apps.itr.util.date import compact_timestamp, today_iso
...
def serialize(m, opts=None):
    ...
    payload = order_by_schema({'ITR': {'ITR1': itr}})
